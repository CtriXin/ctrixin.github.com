...
initShow = function (data) {
    ...
    //var red_buttons = $(".red_buttons")[0].getAttribute("content").split("_");
    //var buttonsInfo = $(".buttons")[0].getAttribute("content").split("_");
...
initLoginShow = function (login_data) {
    ...
    //全部隐藏
    //得到不显示内容
    var deleList = ["sina", "mobile", "qq"];
    var deleObj = "";
    for (var i = 0; i < deleList.length; i++) {
        if (deleList.indexOf(login_data[i]) == -1) {
            deleObj = deleList[i];
        }
    }
    //操作显示
    deleObj = ".login_" + deleObj;
    $(deleObj).hide().siblings().removeClass(deleClass).addClass(showClass);
};
...
weChatAuthorization = function () {
    ...
    SinaSDK.loginData.isStartLogin = true;
    //判断渠道
    //if (Number(SinaSDKTool.data.channel) < 30001 || Number(SinaSDKTool.data.channel) > 30020) {
