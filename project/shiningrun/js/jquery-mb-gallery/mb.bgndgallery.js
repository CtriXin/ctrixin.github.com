...
(function($){
	...
	$.mbBgndGallery ={
		...
		defaults:{
			...
			// for example, if you want to reverse the enter/exit effect once the prev1ious button is clicked:
			// you can change the opt.effect onprev1 event : opt.effect = "slideRight"
			// onnext1:function(opt){opt.effect = "slideLeft"}
			// onprev1:function(opt){opt.effect = "slideRight"}

			onStart:function(){},
			onChange:function(opt,idx){},
			onPause:function(opt){},
			onPlay:function(opt){},
			onnext1:function(opt){},
			onprev1:function(opt){}
		...
		changePhoto:function(url,el){
			...
			var image=$("<img/>").hide().load(function(){
				...
				image
				.css($.mbBgndGallery.normalizeCss(el.opt.effect.enter))
				.show()
				.CSSAnimate({top:0,left:0,opacity:1, transform:"scale(1) rotate(0deg)"},el.opt.effTimer,0,el.opt.effect.enterTiming, function() {
			...
			App.slideText(el.opt.imageCounter+1);
		},
			...
			$(document).on("keydown.bgndGallery",function(e){
				switch(e.keyCode){
					case 32:
						...
						break;
					case 39:
						$.mbBgndGallery.next1(el);
						e.prev1entDefault();

						break;
					case 37:
						$.mbBgndGallery.prev1(el);
						e.prev1entDefault();
		...
		buildControls:function(controls,el){
			...
			var next1=$(controls).find(".next1");
			var prev1=$(controls).find(".prev1");
