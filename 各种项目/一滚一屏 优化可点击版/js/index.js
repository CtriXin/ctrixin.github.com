...
$(document).swipeUp(function(){
	...
	last.row = now.row;
	last.col = now.col;
	//if (last.row != 6 && last.row != 1) { now.row = last.row+1; now.col = 1; pageMove(towards.up);}         //这个是只某一页特定不生效上滚!
    if (last.row != 6 && last.row != 1) { now.row = last.row+1; now.col = 1; pageMove(towards.up);}
});
